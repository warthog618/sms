...
func TestSMSCAddressMarshalBinary(t *testing.T) {
	patterns := []struct {
		...
		{
			"number",
			pdumode.SMSCAddress{Addr: "61409865629", TOA: 0x91},
			[]byte{7, 0x91, 0x16, 0x04, 0x89, 0x56, 0x26, 0xf9},
			nil,
		},
		{
			"number alphabet",
			pdumode.SMSCAddress{Addr: "0123456789*#abc", TOA: 0x91},
			[]byte{9, 0x91, 0x10, 0x32, 0x54, 0x76, 0x98, 0xba, 0xdc, 0xfe},
			nil,
		},
		{
			"alpha",
			pdumode.SMSCAddress{Addr: "messages", TOA: 0xd1},
			nil,
			tpdu.EncodeError("addr", semioctet.ErrInvalidDigit(0x6d)),
		},
		{
			"invalid number",
			pdumode.SMSCAddress{Addr: "6140f98656", TOA: 0x91},
			nil,
...
func TestSMSCAddressUnmarshalBinary(t *testing.T) {
	patterns := []struct {
		...
		},
		{
			...
			pdumode.SMSCAddress{Addr: "bc2a7c1c3797c", TOA: 0xd1},
			9,
			nil,
		},
		{
